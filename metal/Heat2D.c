...
static T2DColor LerpColor(T2DColor a, T2DColor b, float t) {
    ...
    float crf = ar + (static_cast<float>(br - ar) * t);
    float cgf = ag + (static_cast<float>(bg - ag) * t);
    float cbf = ab + (static_cast<float>(bb - ab) * t);
    unsigned cr = static_cast<unsigned>(crf < 0.f ? 0.f : (crf > 255.f ? 255.f : crf));
    unsigned cg = static_cast<unsigned>(cgf < 0.f ? 0.f : (cgf > 255.f ? 255.f : cgf));
    unsigned cb = static_cast<unsigned>(cbf < 0.f ? 0.f : (cbf > 255.f ? 255.f : cbf));
...
void CHeat2DKernel::Render() {
    ...
    const unsigned cellW = (screenW / kSimW) ? (screenW / kSimW) : 1;
    const unsigned cellH = ((screenH - kLegendHeight) / kSimH) ? ((screenH - kLegendHeight) / kSimH) : 1;
